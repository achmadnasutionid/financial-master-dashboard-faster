...
export default function EditErhaTicketPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/erha/${ticketId}`).then(res => res.json()),
    ]).then(([companiesData, billingsData, signaturesData, productsData, ticketData]) => {
      ...
      // Find matching billing
      const matchingBilling = billingsData.find((b: Billing) => b.name === ticketData.billingName)
      if (matchingBilling) {
        setSelectedBillingId(matchingBilling.id)
      }
